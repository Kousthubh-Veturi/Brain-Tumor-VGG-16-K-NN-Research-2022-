...
def train_classifier(model_name:str,layer_name:str,classifier:Any): 
    ...
    dest = Path("artifacts/classifiers/",model_name,layer_name,str(classifier)).with_suffix(".pkl") 
    dest.parent.mkdir(parents=True,exist_ok=True)
...
def test_classifier(model_name:str,layer_name:str,classifier:Any):
    ...
    labels = np.array(labels)
    features = np.array(features)
    location_of_classifier = Path("artifacts/classifiers/",model_name,layer_name,str(classifier)).with_suffix(".pkl") 
